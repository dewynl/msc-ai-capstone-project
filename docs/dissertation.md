...
This research investigates whether neural language models can reliably generate structured educational content, addressing the tension between semantic intelligence and syntactic precision in AI-generated curricula. Through Design Science Research spanning seven architectural iterations, this work demonstrates that task simplification—not model scaling—resolves structural generation bottlenecks. The final architecture employs CodeT5-small (60M parameters) with index-based component selection from semantically ranked candidates, achieving 100% parseable outputs across 32 test cases where previous approaches failed completely.

A five-dimensional pedagogical quality framework reveals distinct performance patterns: emergent strengths in topic diversity (87.3%) and difficulty progression (90.6%), contrasted with persistent prerequisite sequencing challenges (44.8% accuracy). Key contributions include empirical validation that constrained output spaces enable smaller models to achieve reliability through appropriate task formulation, and a measurable quality framework distinguishing soft constraints learnable through training from hard constraints requiring algorithmic enforcement. The work transparently documents limitations—handling only 3-module syllabi, lacking expert validation, and requiring architectural enhancement for prerequisite coherence—demonstrating that systematic failure analysis provides equivalent research value to successful solutions.
