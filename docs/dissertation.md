...
Encoder-decoder models separate input processing from output generation. The encoder processes requirements (domain, difficulty, objectives) whilst the decoder generates structured output. Models like T5 (Raffel et al., 2020) and BART (Lewis et al., 2020) demonstrate this approach through text-to-text frameworks treating all NLP tasks as sequence-to-sequence transformations.
...
**Fine-tuning Strategies:** Traditional full fine-tuning updates all parameters but risks losing general capabilities. Layer-wise approaches freeze lower layers (general linguistic features) whilst adapting higher layers for task-specific patterns (Rogers et al., 2020). Adapter-based methods insert small trainable modules between frozen layers, achieving domain adaptation with 10-100× fewer parameters than full fine-tuning (Houlsby et al., 2019)—valuable for resource-constrained deployments.

**Multi-task and Few-Shot Learning:** Multi-task frameworks train simultaneously on general and educational-specific tasks, maintaining broad capabilities whilst developing specialization (Weller et al., 2022). This proves valuable for educational AI where labeled pedagogical data is limited but general linguistic resources are abundant. Few-shot and meta-learning approaches enable adaptation with minimal examples, essential for deployment across multiple subject domains (Li et al., 2024).
...
**Cross-Domain Challenges:** Models show 30-50% performance degradation when applied to domains different from training data (Li et al., 2024). Hybrid architectures combining neural generation with explicit knowledge representations—prerequisite graphs, concept ontologies, educational frameworks—show improved coherence, suggesting successful educational AI requires integrating learned and structured knowledge.
...
**Direct Preference Optimization:** Traditional RLHF requires training separate reward models with significant computational overhead. Direct Preference Optimization (DPO) treats the language model as an implicit reward model, reducing computational requirements by ~50% whilst maintaining alignment quality (Rafailov et al., 2023). For educational applications, DPO achieves improvements with only 20-100 labeled preference examples (Stanford, 2025)—critical for domains where expert annotation is expensive.
...
**Retrieval-Augmented Generation:** RAG systems augment parametric knowledge with dynamic retrieval from external databases, enabling immediate improvements without retraining (Lewis et al., 2020). The architecture comprises a dense retriever (using semantic similarity search) and a generator conditioning on both input and retrieved information. For educational content, RAG incorporates reference syllabi and pedagogical best practices through semantic search at generation time. Modern retrievers like MPNet (Reimers & Gurevych, 2019) enable nuanced similarity matching beyond keywords.
